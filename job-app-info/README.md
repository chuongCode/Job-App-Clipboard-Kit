...
For everyday changes, click the pencil beside a section heading, update that section's fields, and click **Save**. Pressing Enter also saves the active section; use Shift+Enter to insert a newline. Use **Add +** to add another Experience position or an **Additional** link. Experience positions are sorted by their latest end date when loaded or saved; `Present`, `Current`, and `Now` are treated as ongoing positions and appear first. ISO-style dates such as `2025-06` are recommended. The saved profile is kept locally by `browser.storage.local`.
